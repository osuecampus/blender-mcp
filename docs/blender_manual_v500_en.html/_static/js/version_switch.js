...
class Popover {
  ...
  warnOld(release, all_versions) {
    ...
    if (release < current) {
      ...
      const warning = document.querySelector('template#version-warning')
  ...
  dialogToggle(speed) {
    ...
    if (!this.isOpen) {
      ...
      this.dialog
          .animate({opacity: [0, 1], easing: ['ease-in', 'ease-out']}, speed)
          .finished.then(() => {
            ...
            this.btn.parentNode.addEventListener(
                'focusout', this.focusoutHandlerPrime);
            this.btn.parentNode.addEventListener(
                'mouseleave', this.mouseoutHandlerPrime);
          });
      this.isOpen = true;
    } else {
      ...
      this.btn.parentNode.removeEventListener(
          'focusout', this.focusoutHandlerPrime);
      this.btn.parentNode.removeEventListener(
          'mouseleave', this.mouseoutHandlerPrime);
      this.dialog
          .animate({opacity: [1, 0], easing: ['ease-in', 'ease-out']}, speed)
          .finished.then(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (all_langs.hasOwnProperty(lang)) {
    document.querySelector('#lang-popover').innerHTML = all_langs[lang];
  }
